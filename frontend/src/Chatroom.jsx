...
async function getMessages () {
    ...
    try {
        ...
        const messages = data.map(mess => `Author: ${mess.sender}\n message: ${mess.content}`)
        setMessage(messages)
    }catch(error) {
        //log thrown error
        console.log(error)
  ...
  return (
    <div id="chatroom">
      <div id="messages">
        ...
        {message.slice().reverse().map((mess, index) => (
            <div>
            <p key = {index} style={{ whiteSpace: 'pre-line' }}>{mess}</p>
            <br></br>
            </div>
        ))}
      </div>
      ...
      <form id="send-message" onSubmit = {postMessage}>
        ...
        <select id = 'author' value = {sender} onChange={(e) => setSender(e.target.value)}>
